            ...
            <ul className={cls(styles.list, className)}>
                {data
                    ...
                    .sort((a: IChat, b: IChat) =>
                        compareTimes(a.lastMessage?.createdAt, b.lastMessage?.createdAt)
                    )
                    .map(
                        (item: IChat, index: number): JSX.Element => (
                            <ChatsItem
                                data={item}
                                index={index + 1}
                                key={index}
                                onClick={onClick}
                                onClickRemove={removeChat}
                                className={currentChat?.id === item.id ? styles.active : ""}
                            />
                        )
                    )}
            </ul>
